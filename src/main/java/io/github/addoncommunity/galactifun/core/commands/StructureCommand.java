...
public final class StructureCommand extends SubCommand {
    ...
    public StructureCommand(Galactifun galactifun) {
        super("structure", "Команда для структур", true);
    ...
    public void execute(@Nonnull CommandSender sender, String[] args) {
        ...
        if (args[0].equals("save")) {
            ...
            if (file.exists()) {
                try {
                    ...
                    p.sendMessage(ChatColor.GREEN + "Сохранено как '" + args[1] + "'!");
                } catch (IOException e) {
                    e.printStackTrace();
                    p.sendMessage(ChatColor.RED + "Ошибка сохранения файла! Проверьте консоль!");
                }
            }
            return;
        }
        ...
        if (target == null || target.getType().isAir()) {
            p.sendMessage(ChatColor.RED + "Вы должны выбрать блок!");
            return;
        }

        if (args[0].equals("pos1")) {
            p.getPersistentDataContainer().set(this.pos1, PersistentType.LOCATION, target.getLocation());
            p.sendMessage(ChatColor.GREEN + "Установить pos1 " + toString(target));
            return;
        }

        if (args[0].equals("pos2")) {
            p.getPersistentDataContainer().set(this.pos2, PersistentType.LOCATION, target.getLocation());
            p.sendMessage(ChatColor.GREEN + "Установить pos2 " + toString(target));
            return;
        }

        if (args[0].equals("paste")) {
            if (args.length != 2) {
                p.sendMessage(ChatColor.RED + "Использование: /galactifun paste <name>");
                return;
            }
            ...
            if (saved == null) {
                ...
                if (saved == null) {
                    p.sendMessage(ChatColor.RED + "Неизвестная структура '" + args[1] + "'!");
                    return;
                }
            }

            saved.paste(target, StructureRotation.fromFace(p.getFacing()));
            p.sendMessage(ChatColor.GREEN + "Установлено!");
        }
    }
